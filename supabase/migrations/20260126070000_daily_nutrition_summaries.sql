-- Migration: Phase 3 - Adaptive Daily Targets
-- Creates daily_nutrition_summaries table and trigger for auto-aggregation

-- ============================================================================
-- DAILY NUTRITION SUMMARIES TABLE
-- Stores aggregated daily nutrition data for trend analysis
-- ============================================================================
...
CREATE TABLE daily_nutrition_summaries (
  ...
  -- User's targets for that day (snapshot for historical accuracy)
  ...
  -- Percentages (actual/target * 100) - computed for quick queries
...
CREATE INDEX idx_daily_summaries_user_date ON daily_nutrition_summaries(user_id, date DESC);

-- ============================================================================
-- TRIGGER FUNCTION: Recalculate daily summary
-- Called when food_logs are inserted, updated, or deleted
-- ============================================================================
...
-- ============================================================================
-- TRIGGER: Auto-update summaries on food_logs changes
-- ============================================================================

CREATE TRIGGER food_logs_summary_trigger
...
CREATE POLICY "Users can view own summaries" ON daily_nutrition_summaries
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own summaries" ON daily_nutrition_summaries
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update own summaries" ON daily_nutrition_summaries
  FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete own summaries" ON daily_nutrition_summaries
  FOR DELETE USING (auth.uid() = user_id);

-- ============================================================================
-- HELPER FUNCTION: Get week summaries for a user
-- ============================================================================
...
-- ============================================================================
-- BACKFILL: Populate summaries for existing food_logs
-- ============================================================================
...
SELECT
  fl.user_id,
  fl.log_date,
  COALESCE(p.target_calories, 2000),
  COALESCE(p.target_protein, 150),
  COALESCE(p.target_carbs, 200),
  COALESCE(p.target_fat, 65),
  COALESCE(SUM(fl.calories_logged), 0)::INTEGER,
  COALESCE(SUM(fl.protein_logged), 0)::INTEGER,
  COALESCE(SUM(fl.carbs_logged), 0)::INTEGER,
  COALESCE(SUM(fl.fat_logged), 0)::INTEGER,
  CASE WHEN COALESCE(p.target_calories, 2000) > 0
    THEN ROUND((COALESCE(SUM(fl.calories_logged), 0) / COALESCE(p.target_calories, 2000)::DECIMAL) * 100, 2)
    ELSE 0 END,
  CASE WHEN COALESCE(p.target_protein, 150) > 0
    THEN ROUND((COALESCE(SUM(fl.protein_logged), 0) / COALESCE(p.target_protein, 150)::DECIMAL) * 100, 2)
    ELSE 0 END,
  CASE WHEN COALESCE(p.target_carbs, 200) > 0
    THEN ROUND((COALESCE(SUM(fl.carbs_logged), 0) / COALESCE(p.target_carbs, 200)::DECIMAL) * 100, 2)
    ELSE 0 END,
  CASE WHEN COALESCE(p.target_fat, 65) > 0
    THEN ROUND((COALESCE(SUM(fl.fat_logged), 0) / COALESCE(p.target_fat, 65)::DECIMAL) * 100, 2)
    ELSE 0 END,
  COUNT(*)::INTEGER
FROM food_logs fl
LEFT JOIN profiles p ON p.id = fl.user_id
WHERE fl.log_date IS NOT NULL
GROUP BY fl.user_id, fl.log_date, p.target_calories, p.target_protein, p.target_carbs, p.target_fat
